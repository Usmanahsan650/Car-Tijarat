...
export function RegisteredCars(props){
    ...
    useEffect(()=>{
        ...
        fetch(`${process.env.REACT_APP_EBS_URL}/api/vehicle/vehicle/getRegisteredCars`,{
            method:"POST",
            mode:"cors",
            credentials:"include",
            headers:{
    ...
    let handleSubmit=(e)=>{
      ...
      console.log(data);
      fetch(`${process.env.REACT_APP_EBS_URL}/api/auction/register_for_auction`,{
        method:"POST",
        mode:"cors",
        headers:{
          'Content-Type':'application/json'
    ...
    let items = Registeredcars.map((car) => {
        return (
          <Col sm="12" md="4" lg="3" style={{ "box-shadow": "5px 10px 5px grey"}} key={car.RegNo}>
            <Card>
              <CardImg src={process.env.REACT_APP_EBS_URL+car.Image} height={"200px"}/>
              <CardBody>
                <CardTitle>
